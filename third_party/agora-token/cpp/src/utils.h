...
namespace agora
{
    namespace tools
    {
        ...
        //   unsigned char md[EVP_MAX_MD_SIZE]; uint32_t md_len = 0;
        //   HMAC(EVP_sha256(), appCertificate.data(), appCertificate.length(),
        //        message.data(), message.length(), &md[0], &md_len);
        //   return std::string(reinterpret_cast<char*>(md), signSize);
        //
        // AFTER (CommonCrypto via avc::hmac_sha256):
                                     ...
                                     const std::string &message, uint32_t signSize)
        {
            ...
            std::vector<unsigned char> k(appCertificate.begin(), appCertificate.end());
            std::vector<unsigned char> m(message.begin(), message.end());
            std::vector<unsigned char> mac = avc::hmac_sha256(k, m);
            return std::string(mac.begin(), mac.begin() + signSize);
        }
